...
function generateInnerAccordion(id, description) {
    try {
        ...
        data.forEach((stepData, index) => {
            ...
            const item = createAccordionItem(`${id}-innerAccordion-${index}`, `${stepData.description}`, color, 'inner');
//            const item = createAccordionItem(`#${id}-innerAccordion-${index}`, `${stepData.description}`, color);
//            const item = document.createElement('div');
//            const header = document.createElement('h2');
//            header.className = 'accordion-header';
//
//            const button = document.createElement('button');
//            button.className = `accordion-button ${color}`;
//            button.setAttribute('type', 'button');
//            button.setAttribute('data-bs-toggle', 'collapse');
//            button.setAttribute('data-bs-target', `#${id}-innerAccordion-${index}`);
//            button.setAttribute('aria-expanded', 'false');
//            button.textContent = `${stepData.description}`;
//
...
function handleIncomingEvent(eventData) {
    ...
    if (event.stage == ONBOARD_STAGE){
        ...
        handleSetupStartJob(event);
    } else if (event.stage == VERIFY_START_JOB_STAGE){
        handleVerifyStartJob(event);
